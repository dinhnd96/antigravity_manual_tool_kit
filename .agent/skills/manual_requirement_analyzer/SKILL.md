...
Hệ thống AI khi đọc tài liệu cần xuất ra kết quả bao gồm 3 phần chính:
...
- **Phần C: Bảng Tổng Hợp Test Case Đề Xuất (Test Case Coverage):** Sinh bảng danh sách test case bao phủ 100% tài liệu dựa trên 7 nhóm kỹ thuật test chuyên sâu, có tham chiếu ngược về tài liệu (Traceability).
...
## 4. Phần C: Tổng Hợp Bảng Test Case Đề Xuất (Test Case Coverage)
AI phải sinh ra một bảng tổng hợp danh sách các Test Case nhằm bao phủ 100% nội dung tài liệu. Các Test Case này không cần viết bước chi tiết (Test Steps) nhưng phải nêu rõ tiêu đề (Test Case Title) đủ ý và bắt buộc chia thành 7 nhóm sau:
...
4. **🎨 UI/UX & Field Validation (Giao diện & Xác thực):** Trạng thái component (disabled/enabled khi chưa đủ field), hành vi Dropdown, chống XSS/SQL Injection cơ bản, hành vi phím Enter.
...
Sau khi phân tích xong tài liệu, AI **bắt buộc** phải tự động sinh ra một file **Word (.docx)** chứa toàn bộ nội dung Phần A (Tóm tắt), Phần B (Q&A Table) và Phần C (Bảng Test Case). 
